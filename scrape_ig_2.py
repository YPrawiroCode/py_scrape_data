...
class InstaBot:
    def __init__(self,username,password):
       ...
       #driverPath = 'D:\\chromedriver.exe'
        ...
        #go into signup page
        #self.driver.find_element(By.XPATH, "//span[contains(text(),'Sign')]").click()
        ...
        #user_box.send_keys(username)luania
        # self.browser.find_element_by_name("username").send_keys(username) # Enter the user id in the box
        self.browser.find_element(By.CSS_SELECTOR, "input[name='username']").send_keys(username) # Enter the user id in the box
        # send the password
        # password_box = self.browser.find_element_by_name("password")   # For detecting the password box
        password_box = self.browser.find_element(By.CSS_SELECTOR, "input[name='password']")   # For detecting the password box
        ...
        self.browser.find_element(By.XPATH, "//*[text()='Not now']").click()
        sleep(8)
        # self.browser.find_element(By.XPATH, "//button[text()='Not Now')]").click()
        self.browser.find_element(By.XPATH, "//button[@class='_a9-- _ap36 _a9_1']").click()
    ...
    def following_user(self):
        ...
        scroll_box = self.browser.find_element(By.XPATH, "//div[@class='xyi19xy x1ccrb07 xtf3nb5 x1pc53ja x1lliihq x1iyjqo2 xs83m0k xz65tgg x1rife3k x1n2onr6']/div[1]/div")
        # scroll_box = self.browser.find_element(By.XPATH, "//div[@class='isgrP']")
        # scroll_box = WebDriverWait(self.browser, 2).until(EC.element_to_be_clickable((By.XPATH, "//div[@class='xyi19xy x1ccrb07 xtf3nb5 x1pc53ja x1lliihq x1iyjqo2 xs83m0k xz65tgg x1rife3k x1n2onr6']/div[1]")))
        sleep(5)
        # height variable
        last_ht, ht = 0, 1
        while last_ht != ht:
            print(f"scrolling down {last_ht}")
            last_ht = ht
            sleep(2)
            # scroll down and retrun the height of scroll
            # ht = self.browser.execute_script('arguments[0].scrollTop = arguments[0].scrollTop + arguments[0].scrollHeight;', scroll_box)
            # ht = self.browser.execute_script('arguments[0].scrollTop = arguments[0].scrollHeight', scroll_box)
    ...
    def follower_user(self):
        ...
        # scroll_box= self.browser.find_element(By.XPATH, "//div[@class='PZuss']")
